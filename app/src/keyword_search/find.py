...
def bm25_search(query, top_n=10):
    ...
    with ix.searcher() as searcher:
        ...
        for doc in all_docs:
            # Process headline
            headline_tokens = tokenize(doc.get("headline", ""))
            headline_corpus.append(headline_tokens)
            
            # Process proper nouns
            proper_noun_tokens = tokenize(doc.get("keywords_proper_nouns", ""))
            proper_noun_corpus.append(proper_noun_tokens)
        ...
        # Initialize BM25Plus models
        headline_bm25 = BM25Plus(headline_corpus)
        proper_noun_bm25 = BM25Plus(proper_noun_corpus)

        # Get BM25 scores for headlines
        headline_scores = headline_bm25.get_scores(query_tokens)
        
        # Get BM25 scores for proper nouns - using both query tokens and detected entities
        proper_noun_scores = proper_noun_bm25.get_scores(query_tokens)
        
        # If entities were detected, boost scores for documents with matching proper nouns
        if entities:
            entity_scores = proper_noun_bm25.get_scores(entity_tokens)
            # Combine scores with a boost for entity matches
            for i in range(len(proper_noun_scores)):
                proper_noun_scores[i] += entity_scores[i] * 1.5  # Apply a 1.5x boost for entity matches

        # Combine scores (headline score + weighted proper noun score)
        combined_scores = [
            headline_scores[i] + (proper_noun_scores[i] * 1.2)  # Weight proper noun matches 1.2x
            for i in range(len(headline_scores))
        ]
